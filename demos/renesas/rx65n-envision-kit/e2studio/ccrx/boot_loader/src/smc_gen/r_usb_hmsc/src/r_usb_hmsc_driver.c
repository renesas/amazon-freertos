...
void usb_hmsc_driver_start (uint16_t ip_no)
{
    ...
    for (i = 0; i < USB_MAXSTRAGE; i++)
    {
        if (0 == init_flag)
        {
            g_drive_search_que[i] = USB_NULL;
            memset((void *)&g_usb_hmsc_drvno_tbl[i], 0, sizeof(drive_management_t));
            g_usb_hmsc_drvno_tbl[i].use_flag = USB_NOUSE;
        }
        g_usb_hmsc_sub_class[ip_no][i] = 0;
        g_usb_hmsc_out_pipectr[ip_no][i] = 0;
        g_usb_hmsc_in_pipectr[ip_no][i] = 0;
        memset((void *)&usb_hmsc_trans_data[ip_no][i], 0, sizeof(usb_utr_t));
        memset((void *)&usb_hmsc_receive_data[ip_no][i], 0, sizeof(usb_utr_t));
        memset((void *)&g_usb_hmsc_cbw[ip_no][i], 0, sizeof(usb_msc_cbw_t));
        memset((void *)&g_usb_hmsc_csw[ip_no][i], 0, sizeof(usb_msc_csw_t));
    ...
    g_usb_hmsc_tmp_sub_class[ip_no] = 0;
    memset((void *)&g_usb_hmsc_class_data[ip_no], 0, USB_HMSC_CLSDATASIZE);
    usb_shmsc_data_seq[ip_no] = USB_SEQ_0;
    usb_shmsc_stall_err_seq[ip_no] = USB_SEQ_0;
    usb_hmsc_csw_err_loop[ip_no] = USB_OFF;
    g_usb_hmsc_init_seq[ip_no] = USB_SEQ_0;
    g_usb_hmsc_devaddr[ip_no] = 0;
    g_usb_hmsc_speed[ip_no] = USB_NOCONNECT;
    usb_hmsc_trans_size[ip_no] = 0;
    pusb_hmsc_buff[ip_no] = USB_NULL;
    g_usb_hmsc_cmd_data_length[ip_no] = 0;
    g_p_usb_hmsc_device_table[ip_no] = USB_NULL;
    g_p_usb_hmsc_config_table[ip_no] = USB_NULL;
    g_p_usb_hmsc_interface_table[ip_no] = USB_NULL;
    memset((void *)&g_usb_hmsc_class_control[ip_no], 0, sizeof(usb_utr_t));
    usb_shmsc_process[ip_no] = USB_NULL;
    if (0 == init_flag)
    {
