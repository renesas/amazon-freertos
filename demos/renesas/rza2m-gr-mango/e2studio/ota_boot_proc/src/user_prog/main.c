...
#define BOOT_LOADER_UPDATE_TEMPORARY_AREA_LOW_ADDRESS 	(0x50A00000)	// 更新アプリケーション領域 (temporary area) の先頭アドレス			// RZ/A2M OTA 2020.03.19 //
#define BOOT_LOADER_UPDATE_EXECUTE_AREA_LOW_ADDRESS		(0x50200000)	// 実行アプリケーション領域 (exec area) の先頭アドレス				// RZ/A2M OTA 2020.03.19 //
#define USER_RESET_VECTOR_ADDRESS (0x50200300)	// 0x300まで.rsuファイルのヘッダー部分が入るため このアドレスに移動		// RZ/A2M OTA 2020.03.19 //
...
#if (0)
            {
            	register uint32_t* p_iram = (uint32_t*)0x80038000;
            	while(p_iram<0x80100000)
            	{
            		*p_iram++ = 0;
            	}
            }
