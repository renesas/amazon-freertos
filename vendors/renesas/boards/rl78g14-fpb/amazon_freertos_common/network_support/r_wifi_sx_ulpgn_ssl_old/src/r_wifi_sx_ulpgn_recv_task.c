...
uint8_t g_certificate_file[10][20];
uint8_t g_certificate_no;
uint8_t g_certificate_num=0;
...
static void wifi_analyze_get_certificate_string(uint8_t *pstring)
{
	...
	if('\r'==*pstring)
	{
		...
		while ('\0' != g_certificate_str[unum])
		{
			...
			{
				memcpy(&g_certificate_file[ufnum],&g_certificate_str[start_num],(unum-start_num));
				start_num = unum+2;
				ufnum++;
			}
			unum++;
	...
	else
	{
		if(1==g_number_flg)
		{
			...
			if(1==g_number_flg)
			{
				if (unum==1)
				{
					g_certificate_no = g_certificate_numstr[0] - 0x30;
				}
				else if(unum==2)
				{
					g_certificate_no = (g_certificate_numstr[0] - 0x30)*10 +  (g_certificate_numstr[1] - 0x30);
				}
				else
				{
					g_certificate_no = 0;
    ...
    for( ; ; )
    {
    	...
    	else if(SCI_SUCCESS == sci_ercd)
    	{
    		...
    		if('\n' == data)
    		{
				...
				{
					wifi_analyze_get_certificate_string(presponse_buff);
				}
				/* 2020/3/18 Add 証明書確認 */
		...
		if(NULL != pstr2)
		{
			*pstr2 = '\0';
			if(strlen(pstr2) <= 32)
			{
				...
				{
					strcpy(gp_wifi_ap_results->ssid, (char *)pstr1);
				}
