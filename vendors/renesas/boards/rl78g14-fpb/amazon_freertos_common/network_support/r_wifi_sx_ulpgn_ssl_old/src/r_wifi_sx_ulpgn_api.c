...
static int32_t wifi_execute_at_command(uint8_t serial_ch_id, const uint8_t *ptextstring, uint16_t timeout_ms, wifi_return_code_t expect_code,  wifi_command_list_t command, int32_t socket_no);
...
//static int32_t wifi_dns_query(uint8_t *pdomain_name, uint32_t *pipaddress);
...
*/
wifi_err_t R_WIFI_SX_ULPGN_Connect (const uint8_t *pssid, const uint8_t *ppass, uint32_t security, uint8_t dhcp_enable, wifi_ip_configuration_t *pipconfig)
{
    int32_t ret;
    char *pstr, pstr2;
...
int32_t R_WIFI_SX_ULPGN_GetTcpSocketStatus(uint8_t socket_no)
{
	if(socket_no >= g_wifi_createble_sockets)
	{
	    return -1;
	}
...
int32_t    R_WIFI_SX_ULPGN_SocketCreate(uint32_t type, uint32_t ipversion)
{
	...
	if(WIFI_SUCCESS == api_ret)
	{
		...
		if(WIFI_SUCCESS == api_ret)
		{
			...
			g_wifi_socket[i].ssl_cert_key_id = 0;
			g_wifi_socket[i].ssl_ca_id = 0;
			g_wifi_socket[i].trusted_flag = pdFALSE;
...
wifi_err_t R_WIFI_SX_ULPGN_SocketConnect(int32_t socket_no, uint32_t ipaddr, uint16_t port)
{
	...
	{
		vTaskDelay(200);
		if(1 == g_wifi_socket[socket_no].ssl_flag)
		{
			...
			/* Receive timeout  */
			ret = wifi_execute_at_command(wifi_command_port, "ATNSSL=2,1\r", 10000, WIFI_RETURN_ENUM_OK, WIFI_COMMAND_SET_AUTO_TRANSPARENT_MODE, 0xff);
			if(ret != 0)
			{
				api_ret = WIFI_ERR_MODULE_COM;
				printf("ATNSSL ERROR\r\n");
			...
			if (pdTRUE == g_wifi_socket[socket_no].trusted_flag)
			{
				sprintf((char *)g_wifi_uart[wifi_command_port].command_buff, "ATNSSLLD=2,calist%d.crt,2\r", g_wifi_socket[socket_no].ssl_ca_id);
				ret = wifi_execute_at_command(wifi_command_port, g_wifi_uart[wifi_command_port].command_buff, 10000, WIFI_RETURN_ENUM_OK, WIFI_COMMAND_SET_AUTO_TRANSPARENT_MODE, 0xff);
				if(ret != 0)
				{
					api_ret = WIFI_ERR_MODULE_COM;
					printf("ATNSSLLD_CA ERROR\r\n");
				}
				sprintf((char *)g_wifi_uart[wifi_command_port].command_buff, "ATNSSLLD=2,cert%d.crt,1\r", g_wifi_socket[socket_no].ssl_ca_id);
				ret = wifi_execute_at_command(wifi_command_port, g_wifi_uart[wifi_command_port].command_buff, 10000, WIFI_RETURN_ENUM_OK, WIFI_COMMAND_SET_AUTO_TRANSPARENT_MODE, 0xff);
				if(ret != 0)
				{
					api_ret = WIFI_ERR_MODULE_COM;
			...
			ret = wifi_execute_at_command(wifi_command_port, "ATNSSLCFG=2,4,,0,1\r", 10000, WIFI_RETURN_ENUM_OK, WIFI_COMMAND_SET_AUTO_TRANSPARENT_MODE, 0xff);
			if(ret != 0)
			{
				api_ret = WIFI_ERR_MODULE_COM;
...
int32_t R_WIFI_SX_ULPGN_SocketSend (int32_t socket_no, uint8_t *pdata, int32_t length, uint32_t timeout_ms)
{
    ...
    if(0 == wifi_take_mutex(MUTEX_TX))
	{
        ...
        while(sended_length < length)
        {
            ...
            ercd = R_SCI_Send( g_wifi_uart[wifi_data_port].wifi_uart_sci_handle, (uint8_t *)pdata + sended_length, current_send_length);
            if(SCI_SUCCESS != ercd)
            {
            	break;
            }
...
wifi_err_t R_WIFI_SX_ULPGN_SocketClose(int32_t socket_no)
{
	...
	if(g_wifi_socket[socket_no].socket_create_flag == 1)
	{
		...
		g_wifi_socket[socket_no].ssl_cert_key_id = 0;
		g_wifi_socket[socket_no].ssl_ca_id = 0;
		g_wifi_socket[socket_no].socket_status = WIFI_SOCKET_STATUS_CLOSED;
		g_wifi_socket[socket_no].socket_create_flag = 0;
		g_wifi_socket[socket_no].trusted_flag = pdFALSE;
    }
    return api_ret;
...
wifi_err_t R_WIFI_SX_ULPGN_RegistServerCertificate (uint32_t data_id, uint32_t datatype, const uint8_t *pdata, int32_t length, uint32_t timeout_ms)
{
    ...
    if(0 == wifi_take_mutex(mutex_flag))
	{
		...
		wifi_execute_at_command(wifi_command_port, g_wifi_uart[wifi_command_port].command_buff, 1000, WIFI_RETURN_ENUM_OK, WIFI_COMMAND_SET_SYSFALSH_WRITE_DATA, 0xff);
		...
		while(sended_length < length)
		{
			...
			timeout_init(wifi_command_port, 5000);
			g_wifi_uart[wifi_command_port].tx_end_flag = 0;
			ercd = R_SCI_Send(g_wifi_uart[wifi_command_port].wifi_uart_sci_handle, pdata+sended_length, current_send_length);
			if(SCI_SUCCESS != ercd)
			{
				break;
			}
...
wifi_err_t R_WIFI_SX_ULPGN_DeleteServerCertificate (uint8_t *certname)
{
	int32_t api_ret = WIFI_SUCCESS ;
	int32_t ret;
	wifi_return_code_t result;
    ...
    if(0 == wifi_take_mutex(mutex_flag))
	{
		sprintf((char *)g_wifi_uart[wifi_command_port].command_buff, "ATNSSLCERT=%s,0\r",(char*)certname);
...
wifi_err_t R_WIFI_SX_ULPGN_GetServerCertificate (void)
{
	wifi_err_t api_ret = WIFI_SUCCESS;
	int32_t ret;
	uint8_t mutex_flag;
	...
	uint8_t ufnum=0;
	for(ufnum=0;ufnum<10;ufnum++)
	{
		memset(&g_certificate_file[ufnum],0,sizeof(g_certificate_file[ufnum]));
	}
	g_certificate_no=0;
    ...
    if(0 == wifi_take_mutex(mutex_flag))
	{
		...
		ret = wifi_execute_at_command(wifi_command_port, g_wifi_uart[wifi_command_port].command_buff, 1000, WIFI_RETURN_ENUM_OK, WIFI_COMMAND_SET_SYSFALSH_READ_DATA, 0xff);
...
wifi_err_t R_WIFI_SX_ULPGN_SocketOptRequireTls (int32_t socket_no,uint8_t cert_key_id,uint8_t ca_id,BaseType_t xTrustedFlags)
{
	wifi_err_t api_ret = WIFI_SUCCESS;
	...
	if(WIFI_SUCCESS == api_ret)
	{
		api_ret = R_WIFI_SX_ULPGN_SetSslConfiguration(socket_no, 0, cert_key_id, ca_id, xTrustedFlags);
	}
	return api_ret;
}

wifi_err_t R_WIFI_SX_ULPGN_SetSslConfiguration (int32_t socket_no, uint8_t ssl_type, uint8_t cert_key_id, uint8_t ca_id, BaseType_t flags)
{
	uint8_t mutex_flag;
	wifi_err_t api_ret = WIFI_SUCCESS;
	...
	if(WIFI_SUCCESS == api_ret)
	{
		...
		if (pdTRUE == flags)
		{
			g_wifi_socket[socket_no].trusted_flag = flags;
		}
	}
	if(WIFI_ERR_TAKE_MUTEX != api_ret)
	{
	                            ...
	                            if((now_atustat_sent - previous_atustat_sent) > 0)
								{
									...
									while(previous_socket_store_data_size > 0)
									{
										...
										if(stored_data_size > 0)
										{
											...
											if(SCI_SUCCESS != R_SCI_Receive(g_wifi_uart[wifi_data_port].wifi_uart_sci_handle, temprary_buff, read_data_size))
											{
												ret = -1;
												sequence = 0x80;
												break;
...
static int32_t wifi_execute_at_command(uint8_t serial_ch_id, const uint8_t *ptextstring, uint16_t timeout_ms, wifi_return_code_t expect_code,  wifi_command_list_t command, int32_t socket_no)
{
    ...
    wifi_at_communication_info_t * at_info = &g_wifi_uart[serial_ch_id];
	...
	if(ptextstring != NULL)
	{
        ...
        ercd = R_SCI_Send(at_info->wifi_uart_sci_handle, at_buff, strlen((const char *)at_buff));
#endif
		if(SCI_SUCCESS != ercd)
		{
			return -1;
...
wifi_err_t R_WIFI_SX_ULPGN_CreateCertificate(uint32_t u_id, uint32_t u_num, const uint8_t *p_cert, uint32_t u_certLength)
{
	{
		wifi_err_t api_ret;
		if( (p_cert == NULL) || (u_certLength <= 0 ))
		{
			return  WIFI_ERR_PARAMETER;
		}
		api_ret = R_WIFI_SX_ULPGN_RegistServerCertificate (u_id,u_num, p_cert, u_certLength, 0);
		if(WIFI_SUCCESS != api_ret)
		{
			api_ret = WIRI_ERR_FLASH_WRITE;
		}
		return api_ret;
	}
}

void R_WIFI_SX_ULPGN_UpdateSslId(int32_t socket_no,uint8_t cert_key_id,uint8_t ca_id)
{
	g_wifi_socket[socket_no].ssl_cert_key_id = cert_key_id;
	g_wifi_socket[socket_no].ssl_ca_id = ca_id;
}

wifi_err_t	R_WIFI_SX_ULPGN_EraseAllCertificate(void)
{
	wifi_err_t api_ret;
	uint8_t    certificate_no;
	uint8_t    retry_no;

	api_ret = R_WIFI_SX_ULPGN_GetServerCertificate();
	if (0==g_certificate_no)
	{
		/* Certificate File is nothing */
		return WIFI_SUCCESS;
	}
	for (certificate_no=0;certificate_no<g_certificate_no;certificate_no++)
	{
		api_ret = R_WIFI_SX_ULPGN_DeleteServerCertificate(g_certificate_file[certificate_no]);
		if(WIFI_SUCCESS!=api_ret)
		{
