...
#define MUTEX_TX_DATA (1 << 2)
#define MUTEX_RX_DATA (1 << 3)
...
static SemaphoreHandle_t g_wifi_tx_data_semaphore = NULL;
static SemaphoreHandle_t g_wifi_rx_data_semaphore = NULL;
...
wifi_err_t R_WIFI_SX_ULPGN_Open(void)
{
	...
	if(WIFI_SUCCESS == api_ret)
	{
	    /* Phase 3 Semaphore tx for data initialize */
	    if (g_wifi_tx_data_semaphore != NULL)
	    {
	        vSemaphoreDelete(g_wifi_tx_data_semaphore);
	    }
	    g_wifi_tx_data_semaphore = xSemaphoreCreateMutex();
	...
	if(WIFI_SUCCESS == api_ret)
	{
	    /* Phase 4 Semaphore rx for data initialize */
	    if (g_wifi_rx_data_semaphore != NULL)
	    {
	        vSemaphoreDelete(g_wifi_rx_data_semaphore);
	    }

	    g_wifi_rx_data_semaphore = xSemaphoreCreateMutex();
...
#if 1
		if(0 != (open_phase & 0x40))
		{
			vSemaphoreDelete(g_wifi_rx_data_semaphore);
			g_wifi_rx_data_semaphore = NULL;
		}
		if(0 != (open_phase & 0x80))
		{
			vSemaphoreDelete(g_wifi_tx_data_semaphore);
			g_wifi_tx_data_semaphore = NULL;
		}
#endif
...
int32_t R_WIFI_SX_ULPGN_SendSocket (int32_t socket_number, uint8_t *data, int32_t length, uint32_t timeout_ms)
{
    ...
    if(0 == wifi_take_mutex(MUTEX_TX_DATA))
...
//        	    	wifi_give_mutex(MUTEX_TX);
        	    	wifi_give_mutex(MUTEX_TX_DATA);
        	    	return WIFI_ERR_TAKE_MUTEX;
        		}
                ret = wifi_change_socket_index((uint8_t)socket_number);
//    	    	wifi_give_mutex(MUTEX_RX);
    	    	wifi_give_mutex(MUTEX_TX | MUTEX_RX);
                if(ret != 0)
                {
                    /* Give back the socketInUse mutex. */
//        	    	wifi_give_mutex(MUTEX_TX);
...
//    if(0 == wifi_take_mutex(MUTEX_RX))
	if(0 == wifi_take_mutex(MUTEX_RX_DATA))
    {
		if(ULPGN_USE_UART_NUM == 2)
		{
			if(socket_number != current_socket_index)
...
//				wifi_give_mutex(MUTEX_RX);
//				if(0 != wifi_take_mutex(MUTEX_TX | MUTEX_RX))
				wifi_give_mutex(MUTEX_RX_DATA);
				if(0 != wifi_take_mutex(MUTEX_TX | MUTEX_RX | MUTEX_RX_DATA))
				{
					return WIFI_ERR_TAKE_MUTEX;
				}
				ret = wifi_change_socket_index((uint8_t)socket_number);
...
#if DEBUGLOG ==2
//		printf("Semaphore(RX) Get\n\n");
#endif
	}
	/* */
	if(0 != (mutex_flag & MUTEX_TX_DATA))
	{
		if( xSemaphoreTake( g_wifi_tx_data_semaphore, xMaxSemaphoreBlockTime ) != pdTRUE )
		{
        	return -1;
		}
#if DEBUGLOG ==2
//		printf("Semaphore(TX-DATA) Get\n\n");
#endif
	}

	if(0 != (mutex_flag & MUTEX_RX_DATA))
	{
		if( xSemaphoreTake( g_wifi_rx_data_semaphore, xMaxSemaphoreBlockTime ) != pdTRUE )
		{
			return -1;
		}
#if DEBUGLOG ==2
//		printf("Semaphore(RX-DATA) Get\n\n");
#endif
	}
	/* */
	return 0;
...
//		printf("Semaphore(RX) Give\r\n");
	...
	if(0 != (mutex_flag & MUTEX_TX))
	{
		xSemaphoreGive( g_wifi_tx_semaphore);
#if DEBUGLOG ==2
//		printf("Semaphore(TX) Give\r\n");
#endif
	}
	/*   */
	if(0 != (mutex_flag & MUTEX_RX_DATA))
	{
		xSemaphoreGive( g_wifi_rx_data_semaphore);
#if DEBUGLOG ==2
//		printf("Semaphore(RX-DATA) Give\r\n");
#endif
	}
	if(0 != (mutex_flag & MUTEX_TX_DATA))
	{
		xSemaphoreGive( g_wifi_tx_data_semaphore);
#if DEBUGLOG ==2
//		printf("Semaphore(TX-DATA) Give\r\n");
#endif
	}
	/* */
	return;
