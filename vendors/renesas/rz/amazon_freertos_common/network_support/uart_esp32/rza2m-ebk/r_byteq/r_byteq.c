                        ...
                        uint8_t const       byte)
{
    ...
    hdl->p_buffer[hdl->in_index++] = byte;    /* add byte */
                        ...
                        uint8_t * const     p_byte)
{
    ...
    *p_byte = hdl->p_buffer[hdl->out_index++]; /* get byte */
...
#else
    /* Cast to an appropriate type */
    hdl->p_buffer = NULL;                 /* mark QCB as free */
#endif
...
uint32_t  R_BYTEQ_GetVersion(void)
{
    ...
    uint32_t version = (BYTEQ_VERSION_MAJOR << 16) | BYTEQ_VERSION_MINOR;
    return version;
}
