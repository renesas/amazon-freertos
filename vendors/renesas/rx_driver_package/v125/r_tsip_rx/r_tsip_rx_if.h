 ...
 *         : 31.03.2020 1.09-lib Release library version. Added supprot for ARC4, DH, ECDHE P-512r1.
 *                               DH and ECDHE are supported only RX65N.
 *********************************************************************************************************************/
...
/* RSA HASH type. */
#define R_TSIP_RSA_HASH_MD5                                     (0x01)  /* MD5     */
#define R_TSIP_RSA_HASH_SHA1                                    (0x02)  /* SHA-1   */
#define R_TSIP_RSA_HASH_SHA256                                  (0x03)  /* SHA-256 */

/* For ECC operation. */
#define R_TSIP_ECC_KEY_LENGTH_BYTE_SIZE                     (32u)
#define R_TSIP_ECC_PUBLIC_KEY_MANAGEMENT_INFO1_WORD_SIZE    (4u)
#define R_TSIP_ECC_PUBLIC_KEY_MANAGEMENT_INFO2_WORD_SIZE    (4u)
#define R_TSIP_ECC_PRIVATE_KEY_MANAGEMENT_INFO1_WORD_SIZE   (4u)
#define R_TSIP_ECC_PRIVATE_KEY_MANAGEMENT_INFO2_WORD_SIZE   (12u)
...
#define R_TSIP_ALGORITHM_ID_ENCODED_DATA_BYTE_SIZE          (7u)
...
typedef enum
{
    ...
    TSIP_KEY_INDEX_TYPE_AES128_GCM_FOR_DLMS_COSEM,
    TSIP_KEY_INDEX_TYPE_AES128_KEY_WRAP_FOR_DLMS_COSEM,
} TSIP_KEY_INDEX_TYPE;
...
typedef struct tsip_ecc_public_key_index
{
    ...
    struct
    {
        uint32_t    key_management_info1[R_TSIP_ECC_PUBLIC_KEY_MANAGEMENT_INFO1_WORD_SIZE];
        uint8_t     key_qx[R_TSIP_ECC_KEY_LENGTH_BYTE_SIZE];
        uint8_t     key_qy[R_TSIP_ECC_KEY_LENGTH_BYTE_SIZE];
        uint32_t    key_management_info2[R_TSIP_ECC_PUBLIC_KEY_MANAGEMENT_INFO2_WORD_SIZE];
    }
    value;
...
typedef struct tsip_ecc_private_key_index
{
    ...
    struct
    {
        uint32_t    key_management_info1[R_TSIP_ECC_PRIVATE_KEY_MANAGEMENT_INFO1_WORD_SIZE];
        uint32_t    key_management_info2[R_TSIP_ECC_PRIVATE_KEY_MANAGEMENT_INFO2_WORD_SIZE];
    }
    value;
...
e_tsip_err_t R_TSIP_RSA_IF_HASH(uint8_t *p_mes, uint8_t *p_hash, uint32_t mes_len, uint8_t hash_type);
...
e_tsip_err_t R_TSIP_EcdhInit(tsip_ecdh_handle_t *handle, uint32_t key_type, uint32_t use_key_id);
e_tsip_err_t R_TSIP_EcdhReadPublicKey(tsip_ecdh_handle_t *handle, tsip_ecc_public_key_index_t *public_key_index,
        uint8_t *public_key_data, tsip_ecdsa_byte_data_t *signature, tsip_ecc_public_key_index_t *key_index);
e_tsip_err_t R_TSIP_EcdhMakePublicKey(tsip_ecdh_handle_t *handle, tsip_ecc_public_key_index_t *public_key_index,
        tsip_ecc_private_key_index_t *private_key_index, uint8_t *public_key, tsip_ecdsa_byte_data_t *signature,
        tsip_ecc_private_key_index_t *key_index);
e_tsip_err_t R_TSIP_EcdhCalculateSharedSecretIndex(tsip_ecdh_handle_t *handle,
...
e_tsip_err_t R_TSIP_EcdhKeyDerivation(tsip_ecdh_handle_t *handle, tsip_ecdh_key_index_t *shared_secret_index,
        uint32_t algorithm_id, uint8_t *other_info, uint32_t other_info_length, tsip_aes_key_index_t *key_index);
#if (defined BSP_MCU_RX65N || defined BSP_MCU_RX651)
